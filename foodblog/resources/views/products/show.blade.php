          ...
          <div class="card-body p-4 p-md-5 text-start">
            ...
            <div class="mb-4">
              ...
              <p class="text-dark fs-5 lh-base">
                {{ $product->description ?? 'Nessuna descrizione o dettaglio inserito per questa ricetta.' }}
              </p>
            </div>
            ...
            <div class="pt-4 mt-4 border-top border-light d-flex gap-2">
              ...
              <a href="{{ route('products.edit', $product) }}" class="btn btn-warning fw-bold px-4 py-2 rounded shadow-sm d-inline-flex align-items-center gap-2">
                <i class="bi bi-pencil-square"></i> Modifica questo piatto
              </a>

              <form action="{{ route('products.destroy', $product) }}" method="POST" onsubmit="return confirm('Sei davvero sicuro di voler eliminare questo piatto dal blog in modo permanente?');" class="m-0">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger fw-bold px-4 py-2 rounded shadow-sm d-inline-flex align-items-center gap-2">
                  <i class="bi bi-trash-fill"></i> Elimina dal Blog
                </button>
              </form>
